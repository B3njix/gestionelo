...
import {
  Bar, BarChart, CartesianGrid, Cell, Pie, PieChart, Legend,
  ResponsiveContainer, Tooltip, XAxis, YAxis, Line, LineChart,
} from "recharts";
import { monthlyRevenue, eventsByCategory, topProducts, topCustomers, conversionRate } from "@/lib/mock-data";
...
const COLORS = ["oklch(0.42 0.18 295)","oklch(0.78 0.13 85)","oklch(0.58 0.22 300)","oklch(0.65 0.16 155)","oklch(0.72 0.18 50)"];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-4 p-4 sm:p-6 lg:p-8 lg:grid-cols-2">
        ...
        <Card className="shadow-sm">
          <CardHeader><CardTitle className="text-base">Eventos por categoría</CardTitle></CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={eventsByCategory} dataKey="cantidad" nameKey="tipo" cx="50%" cy="50%" outerRadius={90} label>
        ...
        <Card className="shadow-sm">
          <CardHeader><CardTitle className="text-base">Productos más alquilados</CardTitle></CardHeader>
          <CardContent className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topProducts} layout="vertical" margin={{ left: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="oklch(0.92 0.012 285)" />
                <XAxis type="number" stroke="oklch(0.52 0.03 275)" fontSize={12} />
                <YAxis dataKey="nombre" type="category" stroke="oklch(0.52 0.03 275)" fontSize={11} width={140} />
                <Tooltip />
        ...
        <Card className="shadow-sm">
          <CardHeader><CardTitle className="text-base">Tasa de conversión de cotizaciones</CardTitle></CardHeader>
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={conversionRate}>
                ...
                <Line type="monotone" dataKey="tasa" stroke="oklch(0.58 0.22 300)" strokeWidth={3} dot={{ r: 4 }} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
              ...
              {topCustomers.map((c, i) => (
                <div key={c.id} className="flex items-center justify-between rounded-lg border p-3">
                  <div className="flex items-center gap-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{i + 1}</div>
                    <div>
                      <p className="font-medium">{c.nombre}</p>
                      <p className="text-xs text-muted-foreground">{c.ciudad} · {c.eventosTotales} eventos</p>
                    </div>
                  </div>
