...
function NuevoEvento() {
  ...
  const [evento, setEvento] = useState({
    ...
    lugar: "",
    invitados: 80,
    tipo: "Boda" as (typeof EVENT_TYPES)[number],
  ...
  const visibles = useMemo(
    () => products.filter((p) => p.categoria === categoria).slice(0, 12),
    [categoria],
  );
            ...
            <CardContent className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <Field label="Lugar">
                <Input
                  ...
                  placeholder="Ej. Hotel Sheraton Presidente"
                />
              </Field>
              <Field label="Número de invitados">
                <Input
                  ...
                  onChange={(e) => setEvento({ ...evento, invitados: +e.target.value })}
                />
              </Field>
              <Field label="Tipo de evento">
                <Select
                  value={evento.tipo}
                  onValueChange={(v) => setEvento({ ...evento, tipo: v as typeof evento.tipo })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <Field label="Cliente">
                <Select
                  ...
                  onValueChange={(v) => setEvento({ ...evento, clienteId: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
          ...
          <Card className="shadow-sm">
            <CardHeader>
              ...
              <p className="text-xs text-muted-foreground">
                Selecciona productos por categoría y configúralos.
              </p>
            </CardHeader>
            <CardContent>
                ...
                <TabsList className="flex h-auto flex-wrap gap-1 bg-muted/60 p-1">
                  {CATEGORIES.map((c) => (
                    <TabsTrigger
                      key={c}
                      value={c}
                      className="text-xs data-[state=active]:bg-card data-[state=active]:text-primary"
                    >
                      {c}
                    </TabsTrigger>
                  ))}
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <Button
                          ...
                          variant="outline"
                          className="h-7 w-7"
                          onClick={() => updateQty(i.id, i.cantidad - 1)}
                        >
                          −
                        </Button>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <Button
                  onClick={() => {
                    ...
                    setTimeout(() => navigate({ to: "/eventos" }), 600);
                  }}
          ...
          {configProduct && (
            <>
              ...
              <div className="aspect-[5/3] overflow-hidden rounded-lg bg-muted">
                <img
                  src={configProduct.imagen}
                  alt={configProduct.nombre}
              ...
              <div className="space-y-3">
                {configProduct.variantes &&
                  Object.entries(configProduct.variantes).map(([k, opts]) => (
                    <Field key={k} label={k}>
                      <Select
                        value={configVariants[k]}
                        onValueChange={(v) => setConfigVariants({ ...configVariants, [k]: v })}
                      >
                        <SelectTrigger>
                          <SelectValue />
                        </SelectTrigger>
                ...
                <Field label="Cantidad">
                  <Input
                    ...
                    min={1}
                    value={configQty}
                    onChange={(e) => setConfigQty(Math.max(1, +e.target.value))}
                  />
                </Field>
                <div className="flex items-center justify-between rounded-lg bg-muted/50 p-3 text-sm">
                  <span className="text-muted-foreground">Subtotal del producto</span>
                  <span className="font-semibold text-primary">
                    {formatUSD(configProduct.precio * configQty)}
                  </span>
                </div>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setConfigProduct(null)}>
                  Cancelar
                </Button>
