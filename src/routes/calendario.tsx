        ...
        <Card className="shadow-sm">
          <CardContent className="p-4 sm:p-6">
            ...
            <div className="grid grid-cols-7 gap-1">
              {cells.map((d, idx) => {
                if (d === null)
                  return <div key={idx} className="aspect-square rounded-lg bg-muted/30" />;
                ...
                return (
                  <div
                    key={idx}
                    className={`min-h-24 rounded-lg border bg-card p-1.5 text-left ${isToday ? "border-primary ring-1 ring-primary/30" : ""}`}
                  >
                    <div
                    ...
                    <div className="mt-1 space-y-1">
                      {dayEvents.slice(0, 2).map((e) => (
                        <div
                          key={e.id}
                          className={`truncate rounded px-1.5 py-0.5 text-[10px] border ${statusColor[e.estado]}`}
                          title={e.nombre}
                        >
                          {e.nombre}
                        </div>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {formatDate(e.fecha)} · {e.lugar}
                    </p>
                  </div>
                  <Badge variant="outline" className={statusColor[e.estado]}>
                    {e.estado}
                  </Badge>
                </div>
              ))}
